...
export class AsyncController {
	...
	async createRootAsync(@Body() dto: CreateRootAsyncDto): Promise<RootAsyncResponseDto> {
		const rootAsync = await new RootAsync({ value: dto.value }) as RootAsyncInstance;
	...
	async createResultFromDecorate(@Body() dto: CreateRootAsyncDto): Promise<ResultFromDecorateResponseDto> {
		const rootAsync = await new RootAsync({ value: dto.value }) as RootAsyncInstance;
		const resultDecorate = await rootAsync.ResultFromDecorate(dto.multiplier) as ResultFromDecorateInstance;
	...
	async getResultFromDecorate(
		...
		const rootAsync = await new RootAsync({ value: parseInt(value, 10) }) as RootAsyncInstance;
		const resultDecorate = await rootAsync.ResultFromDecorate(parseInt(multiplier, 10)) as ResultFromDecorateInstance;
	...
	async createSubAsync(@Body() dto: CreateSubAsyncDto): Promise<SubAsyncResponseDto> {
		const syncBase = new SyncBase({ baseValue: dto.baseValue });
		const subAsync = await (syncBase as unknown as { SubAsync: (data: { delay: number; extra: string }) => Promise<SubAsyncInstance> }).SubAsync({
			delay: dto.delay,
			extra: dto.extra,
		}) as SubAsyncInstance;
	...
	async createSubDecorate(@Body() dto: CreateSubAsyncDto): Promise<SubDecorateResponseDto> {
		...
		const subDecorate = await subAsync.SubDecorate(dto.decorateValue) as SubDecorateInstance;
	...
	async getSubDecorate(
		...
		const subDecorate = await subAsync.SubDecorate(decorateValue) as SubDecorateInstance;
